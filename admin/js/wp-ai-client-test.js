(function () {
	...
	var runButton = document.getElementById('abilities-bridge-wp-ai-client-run-tests');
	var statusEl = document.getElementById('abilities-bridge-wp-ai-client-test-status');
	var resultsEl = document.getElementById('abilities-bridge-wp-ai-client-test-results');

	if (!runButton || !statusEl || !resultsEl || !window.abilitiesBridgeWpAiClientTest) {
		return;
	}
	...
	runButton.addEventListener('click', function () {
		...
		statusEl.innerHTML = '<div class="notice notice-info inline"><p>Running WP AI Client diagnostics...</p></div>';
		resultsEl.innerHTML = '';

		var body = new URLSearchParams();
		body.set('action', 'abilities_bridge_run_wp_ai_client_tests');
		body.set('nonce', window.abilitiesBridgeWpAiClientTest.nonce);

		fetch(window.abilitiesBridgeWpAiClientTest.ajaxUrl, {
			method: 'POST',
			headers: {
