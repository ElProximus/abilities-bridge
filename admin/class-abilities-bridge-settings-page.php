...
class Abilities_Bridge_Settings_Page {
	...
	/**
	 * Option-name prefix holding newly generated credentials for one-time
	 * display. One option per user (name suffixed with the user ID) so
	 * concurrent admins cannot overwrite or discard each other's pending
	 * secret.
	 */
	const NEW_CREDENTIALS_OPTION = 'abilities_bridge_new_credentials_pending';

	/**
	 * Option name for the current user's pending one-time credentials.
	 *
	 * @return string
	 */
	private function new_credentials_option_name() {
		return self::NEW_CREDENTIALS_OPTION . '_' . get_current_user_id();
	}
	...
	/**
	 * Store newly generated credentials for one-time display after redirect.
	 *
	 * Uses a non-autoloaded option rather than a transient: with an external
	 * object cache drop-in installed, transients are never written to the
	 * database, so a broken or evicting cache backend silently loses the
	 * secret before the redirected request can display it.
	 *
	 * @param array $credentials Credentials array from generate_credentials().
	 * @return void
	 */
	private function store_new_credentials( $credentials ) {
		$this->prune_expired_new_credentials();

		update_option(
			$this->new_credentials_option_name(),
			array(
	...
	/**
	 * Delete expired pending-credentials options left by users who
	 * generated credentials but never returned to view them.
	 *
	 * @return void
	 */
	private function prune_expired_new_credentials() {
		global $wpdb;
		...
		$option_names = $wpdb->get_col(
			$wpdb->prepare(
				...
				$wpdb->esc_like( self::NEW_CREDENTIALS_OPTION . '_' ) . '%'
			)
		);
		...
		foreach ( (array) $option_names as $option_name ) {
			...
			if ( ! is_array( $stored ) || time() > (int) ( $stored['expires'] ?? 0 ) ) {
	...
	/**
	 * Fetch and consume newly generated credentials for one-time display.
	 *
	 * Returns the credentials only if they belong to the current user, have
	 * not expired, and match the requested MCP profile. The stored option is
	 * deleted once consumed (or when expired/foreign) so the secret is only
	 * ever shown once.
	 *
	 * @param string $profile Expected MCP profile for the calling section.
	 * @return array|null Credentials array or null if none to display.
	 */
	private function take_new_credentials( $profile ) {
		$option_name = $this->new_credentials_option_name();
		$stored      = get_option( $option_name );

		if ( ! is_array( $stored ) || ! isset( $stored['credentials']['client_id'], $stored['credentials']['client_secret'] ) ) {
			return null;
		}

		// Expired, or (belt-and-braces — the option name is already keyed by
		// user ID) generated by another user: discard without displaying.
		if ( time() > (int) ( $stored['expires'] ?? 0 ) || get_current_user_id() !== (int) ( $stored['user_id'] ?? 0 ) ) {
			delete_option( $option_name );
			return null;
		}

		$credentials    = $stored['credentials'];
		$stored_profile = $credentials['profile'] ?? self::MCP_PROFILE_ANTHROPIC;

		// Leave the option in place for the section that matches its profile.
	...
	public function handle_mcp_actions() {
		...
		if ( isset( $_POST['generate_oauth'] ) ) {
			...
			// Store for one-time display after redirect (DB-backed, not a
			// transient: external object caches can silently drop transients).
			$this->store_new_credentials( $credentials );
	...
	public function render_mcp_section() {
		...
		if ( $show_credentials ) {
			$credentials = $this->take_new_credentials( self::MCP_PROFILE_ANTHROPIC );
			if ( $credentials ) {
	...
	public function render_chatgpt_mcp_section() {
		...
		if ( $show_credentials ) {
			$credentials = $this->take_new_credentials( self::MCP_PROFILE_CHATGPT );
			if ( $credentials ) {
				$generated_client_id     = $credentials['client_id'];
				$generated_client_secret = $credentials['client_secret'];
			}
